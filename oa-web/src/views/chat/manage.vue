    ...
    <el-card class="box-card">
      ...
      <el-table :data="tableData" v-loading="loading" stripe border>
        ...
        <el-table-column label="附件" width="80" align="center">
          <template #default="{ row }">
            <el-link v-if="row.attachmentId" type="primary" @click="handleDownload(row)">
              附件
            </el-link>
            <span v-else>-</span>
          </template>
...
import { Plus, Refresh, Search, Edit, View, Delete, Upload, Paperclip } from '@element-plus/icons-vue'
import { getPosts, createPost, updatePost, deletePost } from '@/api/chat'
...
async function loadData() {
  ...
  try {
    ...
    // userInfo 结构: { token: '...', userInfo: { id, realName, username, ... } }
    const info = raw.userInfo || raw
    const currentUserId = info.userId || info.id
    const currentUserName = info.realName || info.username || info.nickName
    if (currentUserId) {
      list = list.filter(p => {
        const pid = p.discussUserId || p.userId
        return pid === currentUserId
      })
    } else if (currentUserName) {
      list = list.filter(p => {
        const pname = p.discussUserName || p.author
        return pname === currentUserName
      })
    }
    console.log('[我的管理] 当前用户:', currentUserId, currentUserName, '筛选结果:', list.length)
...
function handleAdd() {
  isEdit.value = false
  Object.assign(form, { id: null, type: '讨论', title: '', content: '', file: null, fileName: '', attachmentId: null })
...
async function handleDelete(row) {
  ...
  try {
    ...
    loadData()
    if (currentRow.value?.id === id || currentRow.value?.discussId === id) {
